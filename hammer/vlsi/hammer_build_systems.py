...
def build_makefile(driver: HammerDriver, append_error_func: Callable[[str], None]) -> dict:
    ...
    make_text_manual_deps = textwrap.dedent("""
        ####################################################################################
        ## Steps for {mod}
        ####################################################################################
        .PHONY: sim-rtl{suffix} syn{suffix} syn-to-sim{suffix} sim-syn{suffix} syn-to-par{suffix} par{suffix} par-to-sim{suffix} sim-par{suffix} sim-par-to-power{suffix} par-to-power{suffix} power-par{suffix} power-rtl{suffix} sim-rtl-to-power{suffix} sim-syn-to-power{suffix} syn-to-power{suffix} power-syn{suffix} par-to-drc{suffix} drc{suffix} par-to-lvs{suffix} lvs{suffix} syn-to-formal{suffix} formal-syn{suffix} par-to-formal{suffix} formal-par{suffix} syn-to-timing{suffix} timing-syn{suffix} par-to-timing{suffix} timing-par{suffix}
    ...
    if not dependency_graph:
        ...
        if driver.database.get_setting("vlsi.manual_dependencies", {}) == {}:
            # Hierarchical flow
            ...
            def find_module_dependencies(driver: HammerDriver, module: str, step: str) -> List[str]:
                manual_dependencies = driver.database.get_setting("vlsi.manual_dependencies", [])
                if manual_dependencies != []:
                    for deps in manual_dependencies:
                        if deps.get(module) is not None:
                            return deps[module].get(step, [])
                else:
                    return []

            def get_hier_rtl_dependencies(driver: HammerDriver, module: str) -> List[str]:
                rtl_deps = []
                for module_iter, config_iter in driver.get_hierarchical_settings():
                    if module_iter == module:
                        rtl_deps = config_iter.get("synthesis.inputs.input_files", [])
                
                if rtl_deps == []:
                    rtl_deps = driver.database.get_setting("synthesis.inputs.input_files", [])
            ...
            for node, edges in dependency_graph.items():
                ...
                # need to revert this each time
                # syn_deps = "$(HAMMER_DEPENDENCIES)"
                syn_manual_dependencies = find_module_dependencies(driver, node, "syn")
                syn_manual_dependencies += get_hier_rtl_dependencies(driver, node)
                syn_manual_deps = " ".join([os.path.realpath(x) for x in syn_manual_dependencies])
                par_manual_dependencies = find_module_dependencies(driver, node, "par")
                par_manual_dependencies += get_hier_rtl_dependencies(driver, node)
                par_manual_deps = " ".join([os.path.realpath(x) for x in par_manual_dependencies])
                par_to_syn = ""
                if len(out_edges) > 0:
                    syn_deps = os.path.join(obj_dir, "syn-{}-input.json".format(node))
                    p_syn_in = "-p {}".format(syn_deps)
                    ...
                    par_to_syn = textwrap.dedent("""
                        ...
                        hier-par-to-syn-{node}: {syn_manual_deps} {syn_deps}

                        redo-hier-par-to-syn-{node}:
                        \t$(HAMMER_EXEC) {env_confs} {pstring} -o {syn_deps} --obj_dir {obj_dir} hier-par-to-syn
                        """.format(syn_deps=syn_deps, prereqs=prereqs, env_confs=env_confs, pstring=pstring,
                        syn_manual_deps=syn_manual_deps,            
                        obj_dir=obj_dir, node=node))
                else:
                    syn_deps = ""

                output += make_text_manual_deps.format(suffix="-"+node, mod=node, env_confs=env_confs, obj_dir=obj_dir, 
                    syn_deps=syn_deps + " " + syn_manual_deps,
                    par_deps=par_manual_deps,
                    par_to_syn=par_to_syn,
                    p_sim_rtl_in=p_sim_rtl_in, sim_rtl_out=sim_rtl_out, sim_rtl_run_dir=sim_rtl_run_dir,
                    power_sim_rtl_in=power_sim_rtl_in, power_rtl_out=power_rtl_out, power_rtl_run_dir=power_rtl_run_dir,
                    sim_syn_in=sim_syn_in, sim_syn_out=sim_syn_out, sim_syn_run_dir=sim_syn_run_dir,
